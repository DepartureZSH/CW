...
function showCoursesDetails(data){
    ...
    if(localStorage.getItem("role")==="student"){
        ...
        const Details = {
            name: "<div style='display: flex; align-items:center'><h1>" + data.name + " </h1><a id='Edit'>[Edit]</a></div>",
        ...
        EditCourse_init(data);
    }
}

function EditCourse_init(data){
    const edit = document.getElementById('Edit');
    edit.addEventListener('click', ()=>{
        window.location.href = window.location.origin + '/mymodules/editcourse?mCode='+data.mCode;
    })
}
